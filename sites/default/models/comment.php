...
class Comment_Model extends Bl_Model
{
   ...
   * @return boolean
   */
  public function insertComment($uid, $subject, $comment, $nickname = '', $status = 0, $ip = '', $replyid = 0)
  {
    global $db;
    if ($replyid != 0) {
    ...
    $sql = 'INSERT INTO comments SET uid = "' . $db->escape($uid) . '", replyid = "' .$db->escape($replyid). '", nickname = "'.$db->escape($nickname).'",
      subject = "' . $db->escape($subject) . '", comment = "' . $db->escape($comment) . '",status = "'.$db->escape($status).'",
