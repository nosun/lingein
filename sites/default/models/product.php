...
class Product_Model extends Bl_Model
{
	...
	public function getProductSnById($pid){
		global $db;
		$db->select('sn');
		$db->where('pid', $pid);
		$result = $db->get('products');
	...
	public function getPopularCommentedProducts($tid, $limit = -1){
		...
		//first get the product SN from the terms.
		
            $db->select( 'terms.tid, b.pid, b.sn, count(comments.cid) as comments_count, products.name, products.list_price, products.sell_price, products.filepath,products.path_alias');
            $db->from( 'terms' );
            $db->join( 'products' , 'terms.tid = products.directory_tid1 or terms.tid = products.directory_tid2 or terms.tid = products.directory_tid3');
            $db->join( array ('b' => '(select distinct sn, min(pid) as pid from products group by sn)'), 'b.sn = products.sn');
            $db->join( 'products_comments' , 'b.pid = products_comments.pid');
            $db->join( 'comments' , 'products_comments.cid = comments.cid');
            $db->where( 'tid = ' , $tid);
            $db->where( 'comments.status = ' , 1);
            $db->groupby( 'b.pid' );
            $db->orderby( 'comments_count DESC' );
             if ($limit !== -1){
                  $db->limit($limit);
            }
            $result = $db->get();
