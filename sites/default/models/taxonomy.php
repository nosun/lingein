...
class Taxonomy_Model extends Bl_Model
{
 ...
 /**
  ...
  public function saveTermPathAlias($dest ,$tid, $vid)
  {
  	global $db;
  	if ($vid == Taxonomy_Model::TYPE_DIRECTORY) {
  	  $db->delete('path_alias', array('src' => 'product/term/' . $tid));
      $db->insert('path_alias', array('src' => 'product/term/' . $tid, 'dest' => $dest));
  ...
  public function deleteUnderTerm($tid)
  {
    ...
    {
    	$db->delete('path_alias', array('src' => 'product/term/' . $val));
    }
