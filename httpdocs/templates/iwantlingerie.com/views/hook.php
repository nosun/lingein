...
//  $instance->view->assign('helpList', $helpList);
  ...
  $helpList['policies']= $frontInstance->getArticleList(array('type_id'=>'our-policies','status'=>1),1,4);
  $helpList['shopping-guide'] = $frontInstance->getArticleList(array('type_id'=>'shopping-guide','status'=>1),1,4);
  $helpList['payment']= $frontInstance->getArticleList(array('type_id'=>'payment','status'=>1),1,4);
  $helpList['shipping']= $frontInstance->getArticleList(array('type_id'=>'shipping','status'=>1),1,4);
  $helpList['faq']= $frontInstance->getArticleList(array('type_id'=>'faq','status'=>1),1,4);
  $instance->view->assign('helpList', $helpList);
...
function hook_default_index(Default_Controller $instance)
{
  ...
  $instance->view->assign('hotProductList', $hotProductList);
   ...
   //获取新品列表
    $newProductList = $frontInstance->getProductsList(array('termname' =>'New Arrivals'), 1, 8);
    if(isset($newProductList) && $newProductList){
      foreach ($newProductList as $hot){
        $hot->star = widgetCallFunction('fivestars','getaveragestars',$hot->pid);
     }
    }
    $instance->view->assign('newProductList', $newProductList);

  //页脚推荐产品列表
  $recommendProductList = $frontInstance->getProductsListBySpecial(array('termname' =>'Recommended'), 1, 4);
  ...
  $disProductList = $frontInstance->getProductsListBySpecial(array('termname' =>'Discounted'), 1, 4);
...
function hook_user_home($instance){
  ...
  $instance->view->assign('user', $user);
    ...
    $instance->view->assign('templatefile', 'userindex.phtml');
