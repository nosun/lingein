...
var BOLING = {
  ...
  caucluatePayments : function()
  {
      ...
      if(provinceid == null && $('input[name="delivery_or_province"]').val() == ''){
  ...
  init_shipping_and_payment: function(el){
	  el.change(function(){
	  var shipping = $(':radio[name=shipping_method][checked]');
	  var payment = $(':radio[name=payment_method][checked]');
	  if(!shipping.length){
		  $(':radio[name=shipping_method][value="ups"]').attr('checked', true);
	  }
	  if(!payment.length){
		  $(':radio[name=payment_method][value="paypal"]').attr('checked', true);
	  }
  ...
  filter_product : function (el) {
	...
	currencySelect : function () {
		$('#curSelector dd li a.flag').click(function(){
...
//  doWhileExist('select.currencySelect', BOLING.currencySelect);
  doWhileExist('#curSelector dd li a.flag', BOLING.currencySelect);
