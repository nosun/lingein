        ...
        success : function(msg) {
          if (msg == -1) {
            ...
            if(Object.keys(provinceArray).length < 1){
            	...
            	$('#state').css({'display':'table-row'});
            	$('#zoneId').css({'display':'none'});
            }else{
            	$('#state').css({'display':'none'});
            	$('#zoneId').css({'display':'table-row'});
	...
	initialize : function () {
		$('.ajax_qty').blur(function(){
	        ...
	        if (!BOLING.numcheck_(obj)) {
	            var pid = obj.attr('id');
	            var qty = obj.val();
	            BOLING.cart.ajaxCheckInventory(pid, true);
			  ...
			  data: {'cart_item_id': cart_item_id}
				...
				if(result == '0'){
					...
					if(!$('a.checkoutBtn').hasClass('disabled')){
						...
						$('a.checkoutBtn').attr('disabled', 'disabled');
					}
				}else if(parseInt(result) < parseInt(current_input.val())){
					current_td.addClass('disable');
					//current_input.focus().css({'border':'1px solid #cc0000'});
					
					$('.error_notice', current_td).show().text('Only ' + result + ' Items Left.');
					if(!$('a.checkoutBtn').hasClass('disabled')){
						$('a.checkoutBtn').addClass('disabled');
						$('a.checkoutBtn').attr('disabled', 'disabled');
					}
