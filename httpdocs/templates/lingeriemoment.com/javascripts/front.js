...
var BOLING = {
  ...
  productSlide : function(el)
  {
	$('.prodsliderow', el).removeClass('active').addClass('deactive');
	$($('.prodsliderow', el)[0]).removeClass('deactive').addClass('active');  
	$('.prodslide-nav a.flex-prev', el).click(function(){
		var prodslide = $(this).closest('div.prodslide');
		  var rows = $('.prodsliderow', prodslide);
		  var prev_active = 0;
		  for(var i = rows.length - 1; i >= 0; i--){
			  if($(rows[i]).hasClass('active')){
				  $(rows[i]).animate({
				        left: parseInt($(rows[i]).css('left'),10) == 0 ? -$(rows[i]).outerWidth() :0}, 
				        200, function(){
				  ...
				  if(i - 1 >= 0){
					  ...
					  prev_active = rows.length - 1;
				  }
				  break;
			  }
		  }
		  $(rows[prev_active]).removeClass('deactive').addClass('active').css({'right':-$(rows[prev_active]).outerWidth()});
		  $(rows[prev_active]).animate(
				  {right: parseInt($(rows[prev_active]).css('right'),10) == 0 ? -$(rows[prev_active]).outerWidth() :0}, 
		        200, function(){
		        	$(this).attr('style', '');
		  });
		}
	);
	$('.prodslide-nav a.flex-next', el).click(function(){
		var prodslide = $(this).closest('div.prodslide');
		  var rows = $('.prodsliderow', prodslide);
		  var next_active = 0;
		  for(var i = 0; i < rows.length; i++){
			  if($(rows[i]).hasClass('active')){
				  $(rows[i]).animate({
				      right: parseInt($(rows[i]).css('right'),10) == 0 ? -$(rows[i]).outerWidth() :0}, 200, function(){
					  $(this).removeClass('active').addClass('deactive');
					  //$(this).css({'right':'', 'left':''});
					  $(this).attr('style', '');
				  });
		  ...
		  $(rows[next_active]).removeClass('deactive').addClass('active').css({'left':-$(rows[next_active]).outerWidth()});

		  $(rows[next_active]).animate(
				  {left: parseInt($(rows[next_active]).css('left'),10) == 0 ? -$(rows[next_active]).outerWidth() :0}, 200, function(){
					  $(this).attr('style', '');
		  });
		}
		);
  ...
  caucluatePayments : function()
  {
      ...
      */
      ...
      var payment_name = $('select[name=payment_method]').val();
      ...
      $.ajax( {
        ...
        success : function(msg) {
          if (msg == -1) {
            ...
            $('#select_provinces.usebox').selectBox("destroy");
            var select = document.getElementById('select_provinces');
            select.innerHTML = '';
            $('input[name="delivery_or_province"]').val('');
            ...
            if(Object.keys(provinceArray).length < 1){
	            ...
	            if(pid != 'undefined'){
	               $("#select_provinces option[value='"+pid+"']").attr('selected',true);
	            }
	            $('#select_provinces.usebox').selectBox();
            }
          }
        }
      });
  ...
  init_shipping_and_payment: function(el){
	  el.change(function(){
	  var shipping = $(':radio[name=shipping_method][checked]').removeAttr('checked');
	  //var payment = $(':radio[name=payment_method][checked]').removeAttr('checked');
...
BOLING.order = {
		initialize: function(el) {el.find('input.datepicker').datepicker({dateFormat:'yy-mm-dd'});}
};

BOLING.cart = {
  initialize : function () {
    $('.ajax_qty').change(function(){
      var obj = $(this);
      if (!BOLING.numcheck_(obj)) {
        //var old_qty = obj.attr('alt');
        var cart_item_id = obj.attr('id');
        //var price = obj.attr('rel');
        var qty = obj.val();
        //var cartInfo = new Array();

        BOLING.cart.ajaxCheckInventory(cart_item_id, true);
        BOLING.cart.ajaxCartItemUpdate(cart_item_id, qty);
      }  
    });
  ...
  ajaxCartItemUpdate:function(cart_item_id, qty){
    $.ajax( {
	      type : "GET",
          url : url('cart/updateOrderProductQty/' + encodeURI(cart_item_id) + "/" + encodeURI(qty)),
          contentType : "application/txt; charset=utf-8",
          dataType : "json",
          success : function(msg) {
        	  if(msg['error'] != undefined){
        		  ...
        		  if(cart_item_id == msg['modified_item']['cart_item_id']){
        			  //change the value for this product.
        			  $('tr#cartitem_'+ cart_item_id + ' .prQuant input[name="qtys[]"]').val(msg['modified_item']['quantity']);
        			  $('tr#cartitem_'+ cart_item_id + ' .prAmount .payPrice .numVal').text(msg['modified_item']['subtotal']);
        		  }
            	  $('tfoot .scTotalSaving .numVal').text(msg['saving_amount']);
            	  $('tfoot .totalPrice .numVal').text(msg['amount']);
            	  $('.total.goods_number').text(msg['total']);
        	  }
          }
        })
  ...
  ajaxCheckInventory:function(cart_item_id, is_async){
		$.ajax({
			  ...
			  data: {'cart_item_id': cart_item_id}
				...
				var current_input = $('input.ajax_qty[id="'+cart_item_id+'"]');
				var current_td = $('#cartitem_' + cart_item_id);
				if(result == '0'){
					//disable
					current_td.addClass('disable');
					current_input.focus().css({'border':'1px solid #cc0000'});
					$('.error_notice', current_td).show().text('Out of stock.');
					if(!$('a.checkoutBtn').hasClass('disabled')){
						$('a.checkoutBtn').addClass('disabled');
					}
				}else if(parseInt(result) < parseInt(current_input.val())){
					current_td.addClass('disable');
					current_input.focus().css({'border':'1px solid #cc0000'});
					$('.error_notice', current_td).show().text('No enough items.');
					if(!$('a.checkoutBtn').hasClass('disabled')){
						$('a.checkoutBtn').addClass('disabled');
					}
				}else{
					//tell user the storage is ok.
					current_td.removeClass('disable');
					current_input.removeAttr('style');
					$('.error_notice', current_td).text('').hide();
					if(!$('.cart-item-tr.disable').exists()){
						//make the button alive again.
						$('a.checkoutBtn.disabled').removeClass('disabled');
					}
				}
			});
	},
...
(function() {
 ...
 // doWhileExist('select#shipping_method', BOLING.getshippingmoney);
  ...
  doWhileExist('#header_search_keyword', BOLING.searchBox.initialize);
  doWhileExist('#search_keyword', BOLING.searchBox.searchContentCheck);
...
//  doWhileExist('select.currencySelect', BOLING.currencySelect);
  ...
  doWhileExist('select#select_countries', BOLING.init_shipping_and_payment);
  //doWhileExist('select#select_provinces', BOLING.init_shipping_and_payment);
  doWhileExist('.prodslide', BOLING.productSlide);
})();
